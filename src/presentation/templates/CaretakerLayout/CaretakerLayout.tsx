...
const navItems: NavItem[] = [
  ...
  },
  {
    label: 'Mis Pacientes',
    to: '/my-patients',
    icon: <NavIcon><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-full w-full"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2" /><circle cx="9" cy="7" r="4" /><path d="M23 21v-2a4 4 0 0 0-3-3.87" /><path d="M16 3.13a4 4 0 0 1 0 7.75" /></svg></NavIcon>,
  },
  {
    label: 'Alertas',
    to: '/alerts',
    icon: <NavIcon><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-full w-full"><path d="M18 8A6 6 0 0 0 6 8c0 7-3 9-3 9h18s-3-2-3-9" /><path d="M13.73 21a2 2 0 0 1-3.46 0" /></svg></NavIcon>,
  },
  {
    label: 'Vitales',
    to: '/vitals',
    icon: <NavIcon><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-full w-full"><path d="M22 12h-4l-3 9L9 3l-3 9H2" /></svg></NavIcon>,
  },
  {
    label: 'Citas',
    to: '/appointments',
  ...
  },
  {
    label: 'Medicamentos',
    to: '/medications',
    icon: <NavIcon><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="h-full w-full"><path d="M9 3H5a2 2 0 0 0-2 2v4" /><path d="M9 3h10a2 2 0 0 1 2 2v4" /><path d="M3 9h18" /><path d="M3 9v10a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V9" /><line x1="12" y1="13" x2="12" y2="17" /><line x1="10" y1="15" x2="14" y2="15" /></svg></NavIcon>,
  },
  {
    label: 'Mi Perfil',
    to: '/profile',
...
export interface CaretakerLayoutProps {
...
export function CaretakerLayout({ children, title, subtitle, actions }: CaretakerLayoutProps) {
    ...
    <div className="flex min-h-screen bg-gray-50">
      <aside className="fixed inset-y-0 left-0 z-20 flex w-60 flex-col bg-gray-900">
        <div className="flex items-center gap-3 border-b border-white/[0.08] px-5 py-6">
          <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[10px] bg-violet-600">
                ...
                <li key={item.to}>
                  <Link
                    ...
                    className={cn(
                      ...
                      isActive ? 'bg-violet-600 text-white' : 'text-white/60 hover:bg-white/[0.07] hover:text-white',
                    )}
                  >
                    {item.icon}
                    {item.label}
        ...
        <div className="border-t border-white/[0.08] p-4">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 flex-shrink-0 items-center justify-center rounded-[10px] bg-violet-600 text-xs font-bold text-white">{initials}</div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-[13px] font-semibold text-white">{user ? `${user.firstName} ${user.lastName}` : '—'}</p>
              <p className="text-[11px] text-white/40">Cuidador</p>
            </div>
